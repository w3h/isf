...
class OwnFuncField(BaseField):
    ...
    def __init__(self, value, encoder=ENC_STR_DEFAULT, fuzzable=True, name=None, fuzz_count=1000, seed=1024):
        ...
        super(OwnFuncField, self).__init__(value=str(value()), encoder=encoder, fuzzable=fuzzable, name=name)
        # reset random count
        random.seed(self._seed)
    ...
    def reset(self):
        super(OwnFuncField, self).reset()
        # reset fuzz_packet to default status
        random.seed(self._seed)


    def get_info(self):
        info = super(OwnFuncField, self).get_info()
        # add seed to report
